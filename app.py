...
def process_data(store_df, ad_sheets):
    ...
    for sheet_name, sheet_df in ad_sheets.items():
        date_col = None
        for col in ["日", "日付", "年月", "週次"]:
            if col in sheet_df.columns:
                date_col = col
                break
        if date_col:
            sheet_df["日付"] = pd.to_datetime(sheet_df[date_col], errors="coerce")
            ad_dfs.append(sheet_df)
            st.success(f"{sheet_name} シートから日付列 '{date_col}' を使用しました。")
        else:
...
def main():
    st.title("Webマーケ分析アプリ (日付列自動判定付き)")
